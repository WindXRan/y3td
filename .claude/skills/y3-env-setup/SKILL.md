...
description: >
  ...
  检测条件：.claude/env_setup_done 文件不存在时自动触发
---

# Y3 环境配置助手（一次性）

> ⚠️ **一次性 Skill**：完成后创建 `.claude/env_setup_done` 标记，后续不再执行。
...
```
1. 检查 .claude/env_setup_done 是否存在
   ├─ 存在 → ✅ 跳过，继续处理用户请求
   └─ 不存在 → 继续检查步骤 2
...
```bash
echo Environment setup SKIPPED by user on %DATE% %TIME% > .claude\env_setup_done
echo Status: SKIPPED >> .claude\env_setup_done
```

> 💡 用户选择跳过后，如需重新配置，删除 `.claude/env_setup_done` 文件即可。
...
```bash
echo Environment setup completed on %DATE% %TIME% > .claude\env_setup_done
echo Status: OK >> .claude\env_setup_done
echo Python: OK >> .claude\env_setup_done
echo Git: OK >> .claude\env_setup_done
echo y3-lualib: OK >> .claude\env_setup_done
echo AutoSave: DISABLED >> .claude\env_setup_done
```
