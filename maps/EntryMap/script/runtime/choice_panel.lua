...
local function set_ui_text(ui, text)
    if ui and (not ui.is_removed or not ui:is_removed()) and ui.set_text then
...
local function build_choice_subtitle_text(kind, choice)
    ...
    if kind == 'bond' then
        local bond_name = normalize_choice_text(choice.bond_root_name or choice.bond_name or choice.tag or normalize_quality_display(choice.quality))
...
local function apply_choice_card_position(card, selected)
    if not card then
        return
    end
    set_card_position(card, selected)
end
...
local function get_pending_round_choice_kind()
    if STATE.gear_state and STATE.gear_state.awaiting_choice then
        return 'gear'
    elseif STATE.bond_runtime and STATE.bond_runtime.awaiting_choice then
        return 'bond'
    elseif STATE.attr_choice_runtime and STATE.attr_choice_runtime.awaiting_choice then
        return 'attr'
    elseif STATE.evolution_runtime and STATE.evolution_runtime.awaiting_choice then
        return 'evolution'
    end
    return nil
end
...
local function set_card_position(card, selected)
    if not card or not card.set_pos or not card.get_pos then
        return
    end
...
refresh_choice_selected_styles = function()
    ...
    for index = 1, 4 do
        ...
        local is_selected = selected_index == index
        apply_choice_card_style(card, is_selected, quality)
        apply_choice_card_position(card, is_selected)
    end
end
...
local function build_choice_list_cards()
    ...
    for index = 1, 4 do
        ...
        if card and is_visible and choice then
            ...
            local is_selected = selected_index == index
            apply_choice_card_style(card, is_selected, choice.quality)
            apply_choice_card_position(card, is_selected)
        end
    end
