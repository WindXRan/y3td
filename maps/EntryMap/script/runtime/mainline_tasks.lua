...
local function h(i)
  ...
  if j < 100 then
    ...
    local l = j % 10;
    local m = (g[k] or tostring(k)) .. '十'
    if l > 0 then m = m .. (g[l] or tostring(l)) end;
...
function a.create(A)
  ...
  local function M(N)
    ...
    for z, o in ipairs(L()) do
      if o and o.id == N then return z end
    end;
  ...
  local function S()
    B.mainline_task_runtime = B.mainline_task_runtime or {
      active_task_id = nil,
      state = 'idle',
  ...
  local function U(T)
    ...
    local V = O()
    if V then
      T.active_task_id = V.id; T.state = T.state or 'idle'
      return
    end;
    T.chain_exhausted = true; T.state = 'exhausted'
  end;

  local function W()
    if B.session_phase ~= nil then return B.session_phase == 'battle' end;

    return B.current_mode_def ~= nil
  end;
  ...
  local function a4(a5)
    ...
    if a5.type == 'resource' then
      local a6 = f[a5.key] or tostring(a5.key or '?')
      return string.format('%s %s', a6, a1(a0, false))
    end;
    ...
    local a6 = d[a5.key] or e[a5.key] or tostring(a5.key or '?')
    return string.format('%s %s', a6, a1(a0, tostring(a5.key or ''):match('_pct$') ~= nil))
  end;

  local function a7(o)
    local a8 = {}
    for Y, a5 in ipairs(o and o.reward_lines or {})
  ...
  function J.get_current_progress_count(N)
    local T = S()
    local ab = T.progress_by_task_id or {}
    return tonumber(ab[N or J.get_current_task_id()] or 0) or 0
  end;

  function J.is_task_completed(N)
    local T = S()
    return T.completed_task_ids[N] == true
  end;

  function J.is_task_rewarded(N)
    local T = S()
    return T.rewarded_task_ids[N] == true
  end;
  ...
  local function ac(a5, ad)
    ...
    ad[ae] = (ad[ae] or 0) + (tonumber(a5.value) or 0)
    return true
  end;
  ...
  function J.apply_task_rewards(o)
    ...
    do
      if a5.type == 'attr' then
        ac(a5, ad)
      elseif a5.type == 'runtime' then
        af(a5, ad)
    ...
    if o.id then
      T.completed_task_ids[o.id] = true; T.rewarded_task_ids[o.id] = true
    end;
  ...
  function J.start_current_task_challenge()
    ...
    T.progress_by_task_id[o.id] = 0; T.active_challenge = {
      task_id = o.id,
      instance_id = ah.id,
      elapsed = 0,
      time_limit = aa(o),
      ...
      kill_count = 0,
      alive_count = tonumber(o.target_count) or 0
    }
    T.state = 'running'
    T.last_result = 'none'
    T.last_result_reason = nil;
    return true
  end;
  ...
  function J.fail_current_task(ai)
    ...
    T.last_result_reason = ai or 'unknown'
    return true
  end;
  ...
  function J.complete_current_task()
    ...
    T.state = 'completed'
    T.completed_task_ids[o.id] = true;
    if E then E(string.format('%s 已完成。', v(o))) end;
    J.apply_task_rewards(o)
    a9(T)
    local ak = P(o.id)
    if ak then
      T.active_task_id = ak.id; T.state = 'idle'
      T.last_result = 'success'
      T.last_result_reason = nil
    else
      T.active_task_id = nil; T.chain_exhausted = true; T.state = 'exhausted'
      T.last_result = 'success'
      T.last_result_reason = nil
    end;
  ...
  function J.handle_enemy_killed(al)
    ...
    local an = tonumber(o.target_count or aj.target_count or 0) or 0; aj.kill_count = math.min(an, (aj.kill_count or 0) +
    1)
    aj.alive_count = math.max(0, an - aj.kill_count)
    T.progress_by_task_id[o.id] = aj.kill_count;
    return true
  end;
  ...
  function J.update(ao)
    ...
    aj.elapsed = math.max(0, tonumber(aj.elapsed) or 0) + (tonumber(ao) or 0)
    return false
  end;
  ...
  function J.handle_challenge_finished(ah, ap)
    ...
    if ap == true then
      aj.alive_count = 0; aj.kill_count = aj.target_count or aj.kill_count or 0; T.progress_by_task_id[aj.task_id] = aj
      .kill_count;
      return J.complete_current_task()
    end;

    return J.fail_current_task('timeout')
  end;
  ...
  function J.get_current_task_summary()
    ...
    local o = J.get_current_task()
    if not o then
      if T.chain_exhausted or T.state == 'exhausted' then
        return {
          id = nil,
          title_text = '爬塔挑战',
    ...
    local au;
    local av;
    if as == 'running' and aj then
      aq = tonumber(aj.kill_count or aq) or 0;
      ...
      av = '挑战中'
    elseif as == 'exhausted' or T.chain_exhausted == true then
      at = '爬塔挑战全部完成'
      au = ''
      av = '全部完成'
      as = 'exhausted'
    elseif T.last_result == 'failed' then
      at = '本层挑战失败，可再次开启'
      au = '请重新开始当前层挑战'
      av = '可挑战'
      aq = 0
    ...
    return {
      ...
      remaining_seconds = aj and math.max(0, math.ceil((aj.time_limit or ar) - (aj.elapsed or 0))) or ar
    }
  end;
  ...
  function J.get_tracker_state()
    ...
    return {
      auto_track_enabled = T.auto_track_enabled ~= false,
      ...
      last_result_reason = T.last_result_reason
    }
  end;

  function J.toggle_auto_track()
    local T = S()
    T.auto_track_enabled = not (T.auto_track_enabled ~= false)
    if T.auto_track_enabled then
      T.pinned_task_id = nil; T.snapshot_summary = nil
    else
      local ax = J.get_current_task_summary()
      T.pinned_task_id = ax and ax.id or nil; T.snapshot_summary = ax
    end;

    return J.get_tracker_state()
  end;
