...
function M.create(dependencies)
    ...
    local hero_attr_system = dependencies.hero_attr_system
    local message = dependencies.message
    local logger = dependencies.logger('BattleLogic')
    ...
    local function deal_skill_damage(target, amount, damage_info, visual)
        if not target or not target.is_exist or not target:is_exist() then
            logger.warn('Cannot deal damage to non-existent target')
            return 0
        end
        ...
        if damage_info and damage_info.damage_type then
            local multiplier = dependencies.get_damage_bonus_multiplier and 
                dependencies.get_damage_bonus_multiplier(target, damage_info) or 1
        ...
        if visual then
            dependencies.emit_damage_debug_visual and dependencies.emit_damage_debug_visual(visual, target)
        end
    ...
    local function heal_hero(amount)
        if amount <= 0 then
            logger.warn('Heal amount must be positive', { amount = amount })
            return
        end
        
        if not STATE.battle.hero or not STATE.battle.hero:is_exist() then
            logger.warn('Cannot heal non-existent hero')
            return
        end
    ...
    local function get_enemies_in_range(center, radius, except_unit, max_count)
        ...
        return battlefield_system and battlefield_system.get_enemies_in_range(center, radius, except_unit, max_count) or {}
    end
    
    local function get_enemies_on_line(origin_point, impact_point, max_distance, line_width, max_hits, except_unit)
        local battlefield_system = dependencies.battlefield_system
        return battlefield_system and battlefield_system.get_enemies_on_line(origin_point, impact_point, max_distance, line_width, max_hits, except_unit) or {}
    end
    
    local function get_hero_point()
        local hero = get_current_hero()
        if not hero or not hero:is_exist() then
            return STATE.battle.defense_point
        end
    ...
    local function get_bond_runtime_bonus(key)
        ...
        local bond_system = dependencies.bond_system
        local bond_bonus = bond_system and bond_system.get_runtime_bonus(STATE, key) or 0
