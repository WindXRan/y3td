...
function M.create(env)
  ...
  function api.create_hero(basic_attack_range)
    ...
    local initial_attack_range = tonumber(hero_entry_stats['攻击范围']) or basic_attack_range or 250
    if hero_attr_system and hero_attr_system.set_attr then
      pcall(hero_attr_system.set_attr, hero, '攻击范围', math.max(80, math.floor(initial_attack_range * 0.5)))
    end
    ...
    if hero_attr_system and hero_attr_system.get_attr then
      local ok_hp, max_hp = pcall(hero_attr_system.get_attr, hero, '生命结算值')
