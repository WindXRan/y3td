local a=require'ui.ui_root'local bt=require'runtime.bond_tip_model_builder'local QualityImageTable=require'data.tables.economy.quality_image_table'local SkillRuntimeTuning=require'data.tables.skill.skill_runtime_tuning'local BondUiText=SkillRuntimeTuning and SkillRuntimeTuning.bond and SkillRuntimeTuning.bond.ui or{}local BondUiSkillBlockTitle=tostring(BondUiText.skill_block_title or'[����ϵͳ]')local BondUiSkillSectionTemplate=tostring(BondUiText.skill_section_template or'��%s������ϵͳ��')local BondUiSkillSectionFallback=tostring(BondUiText.skill_section_fallback or'�����ܡ�����ϵͳ��')local b={}function b.create(c)local d=c.STATE;
local e=c.y3;
...
if O.set_btn_status_string then O:set_btn_status_string('��̬',R)O:set_btn_status_string('����',R)O:set_btn_status_string('����',R)O:set_btn_status_string('����',R)end end;
...
local function a5(a6)return'BondChoice4'end;
...
if ab=='rare'or ab=='excellent'then return'ϡ��'end;
return'��ͨ'end;
...
if aA==''or aA:find('����',1,true)then aA='����ϵͳ'end;
...
for aG,aq in ipairs(ao)do local aH=a9(aq):gsub('^��ǰ��',''):gsub('��$','')if aH~=''and not aF[aH]then aF[aH]=true;aE[#aE+1]=aH end end;
if#aE<2 then for aG,aq in ipairs(aD)do if#aE>=2 then break end;
local aH=a9(aq):gsub('^��ǰ��',''):gsub('��$','')if aH~=''and not aF[aH]then aF[aH]=true;aE[#aE+1]=aH end end end;ao=aE;
local aI=a9(ay and ay.bond_root_name or'')local aJ=a9(ay and ay.bond_root_progress_text or'')local aA=''
if aI~=''then if aJ~=''then aA=string.format('���ܣ� %s (%s)',aI,aJ)else aA='���ܣ� '..aI end else aA=a9(ay and ay.title_text or'')if aA~=''then aA='���ܣ� '..aA end end;
local aB=a9(ay and(ay.pretty_display_name or ay.display_name or ay.title_text)or'')if aB==''then aB='δ����ս����'end;
...
local aA=aK>0 and string.format('[%s] %s Lv.%d',aa(ay and ay.quality or nil),ak(),aK)or string.format('[%s] %s��׺',aa(ay and ay.quality or nil),ak())
...
return{kind=ac,panel_name=a5(#aY),choices=aY,current_round=aZ,can_refresh=a_>0,disabled_refresh_text='ˢ�����þ�'}end;
...
if ac=='evolution'then local aI=d and(d.evolution_runtime)or nil;
if not aI or aI.awaiting_choice~=true or not aI.current_choices or#aI.current_choices==0 then return nil end;
...
aJ[#aJ+1]='[��������]'
if aK~=''and aM>0 then aJ[#aJ+1]=string.format('����%d�� %s �����Զ�����',aM,aK)elseif aK~=''then aJ[#aJ+1]=string.format('����ͬ���ܵ� %s �����Զ�����',aK)else aJ[#aJ+1]='������ͬ���ܵĿ����Զ�����'end;
local aN={}
...
local aO={}if aK~=''then aO[#aO+1]='���ܣ�'..aK..aL end;
return{kind='bond',title=tostring(aI.item_name_text or ay.title_text or'���ɿ���'),subtitle=table.concat(aO,'  '),body=table.concat(aJ,'\n'),icon=aI.icon_res or ay.icon}end;
...
A(b9,true)
b9:add_fast_event('���-���',function()if k then k(af)end end)
b9:add_fast_event('���-����',function()b4c(Z,af)end)
b9:add_fast_event('���-�Ƴ�',function()b4d()end)
b6[b8]=true
end
end
end;
if b6.refresh_btn~=true then local ba=a2(Z,b7 ..'.refresh_btn')if s(ba)and ba.add_fast_event then A(ba,true)ba:add_fast_event('���-���',function()if j then j()end end)b6.refresh_btn=true end end;
if b6.later_btn~=true then local bb=a2(Z,b7 ..'.later_btn')if s(bb)and bb.add_fast_event then A(bb,true)bb:add_fast_event('���-���',function()if l then l()end end)b6.later_btn=true end end end;
...
if bq.can_refresh~=true then v(ba,bq.disabled_refresh_text or'��ǰ����ˢ��')else if a_>0 then v(ba,string.format('���ˢ�º�ѡ��ʣ��%d�Σ�',a_))else local ae=tonumber(aZ.refresh_paid_count or 0)or 0;v(ba,string.format('ˢ�º�ѡ��%dľ�ģ�',ad(ae)))end end;
...
if#ao==0 then return'��ǰû�п���ʾ���������'end;
...
bH:add_fast_event('���-���',function()d.bond_swallow_panel_visible=false;t(bF('layout'),false)t(bE(),false)b4d()end)
end;
...
return bP end;
local function bJ(bP,bQ,bR,bS,bT,bU,bV,bW)local bX=bI(bP,'�ı�',bT,bU,bR,bS)u(bX,bQ)w(bX,bV)if s(bX)and bX.set_text_color and bW then bX:set_text_color(bW[1]or 255,bW[2]or 255,bW[3]or 255,bW[4]or 255)end;
if s(bX)and bX.set_text_alignment then bX:set_text_alignment(0,8)end;
return bX end;
local function bK(bP,bY,bR,bS,bT,bU,bW)local bX=bI(bP,'ͼƬ',bT,bU,bR,bS)x(bX,bY)if bW then y(bX,bW)end;
return bX end;
...
local function bO(bP,bq,af,cb)local cc=bM(bI(bP,'�սڵ�',184,42,0,0))
if not s(cc)then return end;
A(cc,true)
bK(cc,999,92,21,184,42,cb and{255,212,76,110}or{72,126,190,88})
bJ(cc,bq.pretty_display_name or bq.display_name or bq.title or'����',10,22,118,18,14,cb and{255,235,135,255}or{224,238,255,255})
bJ(cc,bq.progress_text or'0/0',130,22,46,18,13,bq.consumed and{255,214,90,255}or{168,198,230,255})
if cc.add_fast_event then cc:add_fast_event('���-���',function()d.bond_swallow_selected_root_index=af;d.bond_swallow_panel_visible=true;
if F then F()end end)end;
if bP and bP.insert_ui_gridview_comp then bP:insert_ui_gridview_comp(cc,af)end end;
local function bQ(bP,bq,af)local cc=bM(bI(bP,'�սڵ�',76,76,0,0))
if not s(cc)then return end;
A(cc,true)
...
if cc.add_fast_event then cc:add_fast_event('���-����',function()local bn=g and g()or nil;
if bn and bn.show_hover_tip_panel then bn.show_hover_tip_panel(b4b(bq))end end)cc:add_fast_event('���-�Ƴ�',function()b4d()end)end;
...
u(bF('layout.main_frame.detail_panel.detail_title'),bS.title or'δѡ����')
u(bF('layout.main_frame.detail_panel.detail_status'),string.format('%s  %s',tostring(bS.status or'δ����'),tostring(bS.progress or'0/0')))
