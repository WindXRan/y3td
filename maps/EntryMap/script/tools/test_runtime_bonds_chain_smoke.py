...
def main() -> None:
    ...
    smoke_source = (
            ...
            "assert(string.find(defs.bond_body_core.desc.single, '生命值+100', 1, true) ~= nil and string.find(defs.bond_body_core.desc.single, '攻击力+50', 1, true) == nil, 'doc: 体术根节点应改为展示真实首张基础卡效果') "
            "assert(string.find(defs.bond_economy_core.desc.single, '杀敌金币+10%', 1, true) ~= nil and string.find(defs.bond_economy_core.desc.single, '杀敌经验+10%', 1, true) == nil and string.find(defs.bond_economy_core.desc.single, '杀敌加成+10%', 1, true) == nil, 'doc: 经济根节点只应展示第一张基础经济卡') "
            ...
            "local growth_tip_state = { bond_runtime = bonds.create_runtime(), resources = { wood = 0 } } "
            "assert(bonds.unlock_node(growth_tip_state, 'bond_growth_core')) "
            "assert(bonds.unlock_node(growth_tip_state, 'bond_growth_core_agility_up')) "
            "assert(bonds.unlock_node(growth_tip_state, 'bond_growth_core_strength_up')) "
            "assert(bonds.unlock_node(growth_tip_state, 'bond_growth_core_attack_up')) "
            "assert(bonds.unlock_node(growth_tip_state, 'bond_growth_agility')) "
            "assert(bonds.unlock_node(growth_tip_state, 'bond_growth_agility_grant')) "
            "assert(bonds.unlock_node(growth_tip_state, 'bond_growth_agility_growth')) "
            "assert(bonds.unlock_node(growth_tip_state, 'bond_growth_agility_amp')) "
            "local growth_tip = assert(bonds.build_latest_owned_tip_payload(growth_tip_state), 'expected growth tip payload for latest owned node') "
            "assert(type(growth_tip.tip_model) == 'table', 'expected latest owned bond tip payload to expose a tip model') "
            "assert(string.find(growth_tip.tip_model.effect_body_text, '我要玩敏捷', 1, true) == nil, 'transition-only advanced text should stay hidden in bond tip payload') "
            "assert(string.find(growth_tip.tip_model.effect_body_text, '杀敌敏捷+0.1', 1, true) ~= nil, 'bond tip payload should keep the real line effect text') "
            ...
            "assert(defs.bond_body_core_vitality.display_name == '筋骨', 'doc: 体术第三张基础卡应改名为筋骨') "
            "assert(string.find(defs.bond_body_core_vitality.desc.single, '筋骨', 1, true) ~= nil, 'doc: 筋骨节点文案应同步新名称') "
            ...
            "assert(type(def.desc.advanced) == 'string', 'missing advanced desc field for ' .. def.id) "
            ...
            "assert(bonds.can_unlock_node(state, 'bond_growth_demon_hunter') == true, 'growth tier-2 package should keep later same-tier route cards available after picking agility') "
            "assert(bonds.can_unlock_node(state, 'bond_growth_agility_grant') == true, 'growth tier-2 package should keep base same-tier cards available after picking agility') "
            "assert(bonds.can_unlock_node(state, 'bond_growth_bow_god') == false, 'growth tier-3 should still stay locked while tier-2 is active') "
            ...
            "bonds.notify_attack_skill_cast({ STATE = state }, { id = 'sword_wave' }, nil) "
            ...
            "assert(string.find(root_choice.current_text, '生命值+100', 1, true) ~= nil and string.find(root_choice.current_text, '攻击力+50', 1, true) == nil, 'root choice body should show the real first root card text') "
            ...
            "assert(bonds.can_unlock_node(body_root_state, 'bond_body_core_vitality') == true, 'body current stage should expose the whole tier-1 package together') "
            "local initial_body_candidates = {} "
            "for _, def in ipairs(bonds.get_candidate_nodes(body_root_state)) do "
            "initial_body_candidates[def.id] = true "
            "end "
            "assert(initial_body_candidates['bond_body_core_momentum'] == true and initial_body_candidates['bond_body_core_vitality'] == true, 'body tier-1 package should be seeded into the candidate pool together') "
            ...
            "assert(bonds.can_unlock_node(body_root_state, 'bond_body_core_momentum') == true and bonds.can_unlock_node(body_root_state, 'bond_body_core_vitality') == true, 'unlocking one body tier-1 card should keep the rest of the current package available') "
            "assert(bonds.can_unlock_node(body_root_state, 'bond_body_life') == false, 'body tier-2 package should stay locked until tier-1 root set is complete') "
            ...
            "assert(bonds.can_unlock_node(body_root_state, 'bond_body_life') == true and bonds.can_unlock_node(body_root_state, 'bond_body_life_blessing') == true and bonds.can_unlock_node(body_root_state, 'bond_body_life_power') == true, 'completing the base body set should expose the whole current tier-2 package instead of only branch starters') "
            "assert(bonds.can_unlock_node(body_root_state, 'bond_body_tactics') == true and bonds.can_unlock_node(body_root_state, 'bond_body_tactics_blunt_edge') == true and bonds.can_unlock_node(body_root_state, 'bond_body_fortress_hard_armor') == true, 'body tier-2 follow-up cards should unlock together with the current stage package') "
            "local body_tier_two_candidates = {} "
            "for _, def in ipairs(bonds.get_candidate_nodes(body_root_state)) do "
            "body_tier_two_candidates[def.id] = true "
            "end "
            "assert(body_tier_two_candidates['bond_body_life_power'] == true and body_tier_two_candidates['bond_body_tactics_blunt_edge'] == true, 'body tier-2 candidate pool should include deep same-tier cards immediately') "
            "assert(body_tier_two_candidates['bond_body_blood_demon'] ~= true and bonds.can_unlock_node(body_root_state, 'bond_body_blood_demon') == false, 'body tier-3 package should stay locked while tier-2 is still the current stage') "
            "math.random = old_random "
            "local growth_draw_state = { bond_runtime = bonds.create_runtime(), resources = { wood = 999 }, bond_draw_count = 0 } "
            "assert(bonds.can_unlock_node(growth_draw_state, 'bond_growth_core') == true and bonds.can_unlock_node(growth_draw_state, 'bond_growth_core_agility_up') == true and bonds.can_unlock_node(growth_draw_state, 'bond_growth_core_strength_up') == true and bonds.can_unlock_node(growth_draw_state, 'bond_growth_core_attack_up') == true, 'growth tier-1 package should be fully available from the start') "
            "assert(bonds.can_unlock_node(growth_draw_state, 'bond_growth_agility') == false, 'growth tier-2 should stay locked before the root set is complete') "
            ...
            "assert(bonds.can_unlock_node(growth_draw_state, 'bond_growth_agility') == true and bonds.can_unlock_node(growth_draw_state, 'bond_growth_agility_growth') == true and bonds.can_unlock_node(growth_draw_state, 'bond_growth_demon_hunter_hunt') == true, 'completing the base growth set should expose the whole agility tier-2 package together') "
            "assert(bonds.can_unlock_node(growth_draw_state, 'bond_growth_strength') == true and bonds.can_unlock_node(growth_draw_state, 'bond_growth_barbarian_warcry') == true and bonds.can_unlock_node(growth_draw_state, 'bond_growth_archmage_burst') == true, 'growth tier-2 should unlock the whole cross-branch package together') "
            "assert(bonds.can_unlock_node(growth_draw_state, 'bond_growth_bow_god') == false and bonds.can_unlock_node(growth_draw_state, 'bond_growth_war_god') == false, 'growth tier-3 should stay locked while tier-2 is still the current stage') "
            ...
            "  assert(string.sub(choice.node_id or '', 1, 8) ~= '__group_', 'bond draw should not expose group placeholder ids') "
            "end "
            "assert(saw_group_choice == false, 'bond draw should not include group choices when include_group_choices is disabled') "
            "local first_draw_choice = assert(draw_state.bond_runtime.current_choices[1]) "
            "assert(first_draw_choice.node_id ~= nil, 'first bond draw should expose real bond nodes') "
            ...
            "assert(type(draw_choice.effect_text) == 'string') "
            ...
            "projectile = { create = function() return { is_exist = function() return false end, remove = function() end, set_facing = function() end, mover_target = function() end } end }, "
            ...
            "collect_bond_route_tags = function() return { sword_wave = true } end, "
            "ATTACK_SKILL_DEFS = require('data.object_tables.attack_skills').defs_by_id, "
            "ATTACK_SKILL_BLUEPRINTS = require('data.object_tables.attack_skill_second_batch_blueprints') "
            "}) "
            "upgrade_sys.show_upgrade_choices() "
            "math.random = old_random "
            "local has_unlock = false "
            "for _, choice in ipairs(upgrade_state.current_upgrade_choices or {}) do "
            "if choice and type(choice.key) == 'string' and string.sub(choice.key, 1, 7) == 'unlock_' then "
            "has_unlock = true "
            "break "
            "end "
            "end "
            "assert(has_unlock == true) "
            ...
            "assert(panel_model.cards[1].render_prefab == nil, 'bond choice cards should not request dedicated bond prefab rendering') "
            "assert(panel_model.cards[1].use_item_desc_card == true, 'bond choice cards should opt into item desc rendering') "
    ...
    assert_not_contains(bonds_chain_content, "title_color = 'bond_red'", 'bonds_chain.lua should not hardcode bond title colors anymore')
    assert_not_contains(bonds_chain_content, "subtitle_color = 'blue'", 'bonds_chain.lua should not hardcode bond subtitle colors anymore')
    assert_not_contains(bonds_chain_content, 'BondAliasConfig', 'bonds_chain.lua should not depend on alias config loader anymore')
    assert_not_contains(bonds_chain_content, 'build_runtime_alias_pack', 'bonds_chain.lua should use unified runtime keys directly')
