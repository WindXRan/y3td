...
def main() -> None:
    ...
    smoke_source = (
            ...
            "assert(defs.bond_magic_elementalist.parent_id == 'bond_magic_haste', 'xmind: 元素师应接在急速后面') "
            "assert(defs.bond_magic_mage.parent_id == 'bond_magic_haste', 'xmind: 魔法师应接在急速后面') "
            ...
            "assert(string.find(defs.bond_economy_greed.desc.single, '6000', 1, true) ~= nil and string.find(defs.bond_economy_greed.desc.single, '150', 1, true) ~= nil, '贪婪应同步文档中的金币和木材文案') "
            ...
            "assert(string.find(defs.bond_body_life.desc.advanced, '生命增幅', 1, true) ~= nil, '生命节点应同步总表中的生命套装效果') "
            ...
            "assert(defs.bond_body_life.parent_id == 'bond_body_core_vitality', 'xmind: 生命应接在基础体术链完成后') "
            "assert(defs.bond_body_blood_oath.parent_id == 'bond_body_life', 'xmind: 血誓应接在生命后面') "
            "assert(defs.bond_body_blood_demon.parent_id == 'bond_body_blood_oath', 'xmind: 血魔应接在血誓后面') "
            "assert(#defs.bond_body_life.next_ids == 1 and defs.bond_body_life.next_ids[1] == 'bond_body_blood_oath', 'xmind: 生命后继只能是血誓') "
            ...
            "assert(defs.bond_body_charge_breaker.parent_id == 'bond_body_fortress', 'xmind: 陷阵应接在固守后面') "
            "assert(string.find(defs.bond_body_blood_oath.desc.single, '每100点生命上限', 1, true) ~= nil or string.find(defs.bond_body_blood_oath.desc.single, '每有100点生命上限', 1, true) ~= nil or string.find(defs.bond_body_blood_oath.desc.advanced, '每100点生命上限', 1, true) ~= nil or string.find(defs.bond_body_blood_oath.desc.advanced, '每有100点生命上限', 1, true) ~= nil, '血誓节点应体现生命上限转属性') "
            "assert(string.find(defs.bond_body_blood_oath.desc.single, '生命值 +500', 1, true) ~= nil or string.find(defs.bond_body_blood_oath.desc.single, '生命值+500', 1, true) ~= nil or string.find(defs.bond_body_blood_oath.desc.advanced, '生命值 +500', 1, true) ~= nil or string.find(defs.bond_body_blood_oath.desc.advanced, '生命值+500', 1, true) ~= nil, '血誓节点应体现生命值+500套装结果') "
            ...
            "assert(string.find(progress_lines[1], '5/13', 1, true) ~= nil, 'bond progress lines should include subtree progress') "
            ...
            "assert(root_choice.subtitle_text == '', 'root choice should not duplicate the set name in subtitle') "
            "assert(root_choice.title_color == 'bond_red', 'bond title should use the configured red title color') "
            "assert(string.find(root_choice.current_text, '生命值 + 100', 1, true) ~= nil, 'root choice top block should show the base attribute gain from docs') "
            "assert(string.find(root_choice.current_text, '斗气场域', 1, true) == nil, 'root choice top block should no longer reuse set-effect text') "
            "assert(root_choice.body_blocks[1].text == '生命值 + 100\\n力量 + 50', 'multiple value attributes should be split into separate lines') "
            ...
            "assert(string.find(momentum_choice.title_text or '', '气势', 1, true) ~= nil, 'momentum choice title should show 气势') "
            "assert(string.find(momentum_choice.title_text or '', '体术', 1, true) == nil, 'momentum choice title should not be replaced by 体术') "
            ...
            "assert(bonds.can_unlock_node(root_draw_state, 'bond_body_life') == true or bonds.can_unlock_node(root_draw_state, 'bond_body_tactics') == true or bonds.can_unlock_node(root_draw_state, 'bond_body_fortress') == true, 'body branches should appear only after base body set is complete') "
            ...
            "local growth_candidates = bonds.get_candidate_nodes(growth_draw_state) "
            "local agility_index = nil "
            "for i, def in ipairs(growth_candidates) do "
            "if def.id == 'bond_growth_agility' then agility_index = i break end "
            "end "
            "assert(agility_index ~= nil, 'growth base chain complete should expose agility candidate') "
            "local old_random_for_growth = math.random "
            "math.random = function(n) if n == #growth_candidates then return agility_index end return 1 end "
            "assert(bonds.try_draw({ STATE = growth_draw_state, message = function() end })) "
            "math.random = old_random_for_growth "
            "local agility_choice = assert(growth_draw_state.bond_runtime.current_choices[1]) "
            "assert(agility_choice.node_id == 'bond_growth_agility', 'forced growth draw should pick bond_growth_agility first') "
            "assert(string.find(agility_choice.title_text or '', '0/3', 1, true) ~= nil, 'growth branch root title should show its own branch progress instead of root set progress') "
            "assert((agility_choice.effect_title or '') == '', 'branch choice should not show root set preview in the lower block') "
            "assert((agility_choice.effect_text or '') == '', 'branch choice should not show root set preview text in the lower block') "
            "assert(string.find(agility_choice.body_blocks[1].text or '', '\\n', 1, true) ~= nil, 'named card sentences should be split into multiple lines for readability') "
            "assert(agility_choice.body_blocks[1].segments and agility_choice.body_blocks[1].segments[1] and agility_choice.body_blocks[1].segments[1].color == 'gold', 'card name prefix should be highlighted for readability') "
            "assert(agility_choice.body_blocks[2] and agility_choice.body_blocks[2].kind == 'effect_title' and string.find(agility_choice.body_blocks[2].text or '', '本卡', 1, true) ~= nil, 'branch choice should show its own effect title') "
            "assert(agility_choice.body_blocks[3] and string.find(agility_choice.body_blocks[3].text or '', '我要玩敏捷', 1, true) ~= nil, 'branch choice should show its own advanced text') "
            ...
            "assert(bonds.unlock_node(archery_state, 'bond_archery_barrage')) "
            "local archery_candidates = bonds.get_candidate_nodes(archery_state) "
            "local moon_blade_index = nil "
            "for i, def in ipairs(archery_candidates) do "
            "if def.id == 'bond_archery_moon_blade' then moon_blade_index = i break end "
            "end "
            "assert(moon_blade_index ~= nil, 'archery barrage unlock should expose moon blade candidate') "
            "local old_random_for_moon_blade = math.random "
            "math.random = function(n) if n == #archery_candidates then return moon_blade_index end return 1 end "
            "assert(bonds.try_draw({ STATE = archery_state, message = function() end })) "
            "math.random = old_random_for_moon_blade "
            "local moon_blade_choice = assert(archery_state.bond_runtime.current_choices[1]) "
            "assert(moon_blade_choice.node_id == 'bond_archery_moon_blade', 'forced archery draw should pick bond_archery_moon_blade') "
            "assert(string.find(moon_blade_choice.title_text or '', '1/2', 1, true) ~= nil, 'moon blade title should show current branch-chain progress instead of completed root-set progress') "
            "assert(string.find(moon_blade_choice.title_text or '', '箭术', 1, true) == nil, 'moon blade title should not fall back to root-set title text') "
            ...
            "assert(bonds.unlock_node(magic_draw_state, 'bond_magic_haste')) "
            "local magic_candidates = bonds.get_candidate_nodes(magic_draw_state) "
            "local mage_index = nil "
            "for i, def in ipairs(magic_candidates) do "
            "if def.id == 'bond_magic_mage' then mage_index = i break end "
            "end "
            "assert(mage_index ~= nil, 'magic haste unlock should expose mage candidate') "
            "local old_random_for_mage = math.random "
            "math.random = function(n) if n == #magic_candidates then return mage_index end return 1 end "
            "assert(bonds.try_draw({ STATE = magic_draw_state, message = function() end })) "
            "math.random = old_random_for_mage "
            "local mage_choice = assert(magic_draw_state.bond_runtime.current_choices[1]) "
            "assert(mage_choice.node_id == 'bond_magic_mage', 'forced magic draw should pick bond_magic_mage') "
            "assert(string.find(mage_choice.title_text or '', '1/2', 1, true) ~= nil, 'mage title should show current branch-chain progress instead of completed root-set progress') "
