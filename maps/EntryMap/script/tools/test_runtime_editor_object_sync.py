...
from runtime_editor_object_sync_defs import *
...
def get_first_stage_item(items):
...
def get_first_visible_stage_effect(effect_field):
    if isinstance(effect_field, dict):
        return get_first_stage_item(effect_field.get("items"))
    if isinstance(effect_field, list) and effect_field:
        return get_first_stage_item(effect_field)
    return 0
...
def main():
    ...
    assert get_primary_projectile_effect(basic_attack_projectile_data["effect_foes"]) == 104656, (
        "basic attack projectile should keep the unified arrow flight effect"
    )
    assert get_primary_projectile_effect(basic_attack_projectile_data["effect_friend"]) == 104656, (
