...
def main() -> None:
    smoke = (
        ...
        "end "
        "assert(moon_blade_index ~= nil, 'moon blade should be unlockable after barrage') "
        ...
        "assert(choice.node_id == 'bond_archery_moon_blade', 'expected moon blade choice') "
        "assert(string.find(choice.title_text or '', '1/2', 1, true) ~= nil, 'moon blade should use branch progress 1/2 before unlock') "
        "assert(string.find(choice.title_text or '', '3/3', 1, true) == nil, 'moon blade should not show root-set progress 3/3') "
