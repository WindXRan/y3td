...
def main() -> None:
    smoke = (
        ...
        "CONFIG = { unit_ids = { hero = 201390301 }, hero_init_stats = { ['生命'] = 900, ['攻击范围'] = 2000 }, debug_time_scale = 1, debug_hero_bonus_stats = {} }, "
        "y3 = { object = { unit = { [134274912] = { data = {} } } }, ltimer = { wait = function(_, fn) if fn then fn() end end } }, "
        ...
        "  local calls = 0 "
        "  return { "
        "    create_unit = function(_, unit_id, _, _) "
        "      calls = calls + 1 "
        "      if calls == 1 then error('ea_prefab_not_exist') end "
        "      local hero = { attrs = {}, hp = 0, unit_id = unit_id } "
        ...
        "assert(hero.unit_id == 134274912, 'expected hero creation to fallback to base archer unit, got ' .. tostring(hero.unit_id)) "
        "assert(hero:get_attr('攻击范围') == 2000, 'expected fallback hero to still receive init attack range') "
        "print('battlefield create_hero runtime fallback smoke ok') "
    )
    ...
    assert_ok(result, 'battlefield create_hero runtime fallback smoke failed')
