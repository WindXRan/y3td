...
def main() -> None:
    smoke_source = (
        ...
        "assert(#bar_counts >= 1 and bar_counts[#bar_counts] >= 1, 'hero should expose at least one item bar slot for the growth weapon') "
        "assert(#granted_items == 1, 'hero should receive exactly one growth weapon item on stage start') "
        "assert(granted_items[1].item_key == 134250249, 'growth weapon should use the configured item key') "
