...
def main() -> None:
    smoke = (
        ...
        "assert(math.abs(system.get_damage_multiplier(hero, '物理', 'normal_attack') - 1.03) < 0.0001) "
        "assert(math.abs(system.get_damage_multiplier(hero, '法术', 'skill') - 1.02) < 0.0001) "
        "assert(system.get_attr(hero, '攻击结算值') > 107) "
        "assert(math.abs(system.get_attr(hero, '生命结算值') - 316.25) < 0.0001) "
