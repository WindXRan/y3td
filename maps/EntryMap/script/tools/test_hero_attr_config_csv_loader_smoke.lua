...
assert(type(cfg.hero_init_stats['多重伤害']) == 'number', 'hero init multishot damage should remain numeric')
...
assert(cfg.hero_init_stats['攻击白字'] == 60, 'hero init attack should stay intact')
assert(cfg.hero_init_stats['生命白字'] == 600, 'hero init life should stay intact')
assert(cfg.hero_init_stats['护甲白字'] == 6, 'hero init armor should stay intact')
...
assert(cfg.panel_default_attrs['多重伤害'] == 100, 'panel default multishot damage should start at 100')
assert(cfg.hero_init_stats['多重伤害'] == 100, 'hero init multishot damage should start at 100')
