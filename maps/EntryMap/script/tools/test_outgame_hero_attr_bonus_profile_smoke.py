...
def main() -> None:
    smoke = (
        ...
        "assert(profile.hero_attr_bonus_stats['攻击白字'] == 6, 'expected cleared stage attack bonus to apply') "
        "assert(profile.hero_attr_bonus_stats['生命白字'] == 120, 'expected cleared stage hp bonus to apply') "
        "assert(profile.hero_attr_bonus_stats['攻击范围'] == 50, 'expected cleared challenge stage range bonus to apply') "
        "assert(profile.hero_attr_bonus_stats['力量白字'] == nil, 'expected no talent strength bonus') "
        "assert(profile.hero_attr_bonus_stats['敏捷白字'] == nil, 'expected no talent agility bonus') "
        "assert(profile.hero_attr_bonus_stats['智力白字'] == nil, 'expected no talent intelligence bonus') "
