...
def main() -> None:
    smoke = (
        ...
        "  }, "
        "  archive_rewards = { talent_points = 0, talents = { output_1 = 2, survival_1 = 1, survival_7 = 1 } } "
        "} "
        ...
        "assert(profile.hero_attr_bonus_stats['攻击白字'] == 26, 'expected cleared stage and talent attack bonus to apply') "
        "assert(profile.hero_attr_bonus_stats['生命白字'] == 200, 'expected cleared stage and talent hp bonus to apply') "
        "assert(profile.hero_attr_bonus_stats['攻击范围'] == 50, 'expected cleared challenge stage range bonus to apply') "
        "assert(profile.hero_attr_bonus_stats['力量白字'] == 10, 'expected all-attribute talent strength bonus to apply') "
        "assert(profile.hero_attr_bonus_stats['敏捷白字'] == 10, 'expected all-attribute talent agility bonus to apply') "
        "assert(profile.hero_attr_bonus_stats['智力白字'] == 10, 'expected all-attribute talent intelligence bonus to apply') "
