...
function M.create(params)
  ...
  local bond_ui_cfg = (CONFIG.bond_skill_runtime_tuning and CONFIG.bond_skill_runtime_tuning.ui)
      ...
      or {}
  local bond_skill_label = tostring(bond_ui_cfg.hud_skill_title or bond_label_cfg.effect_name or '缇佺粖鎶€鑳?)
  local y3 = params.y3;
  ...
  -- UI 宸ュ叿鍑芥暟 (from ui.hud.hud_core)
  ...
  local function get_hero_hp_info()
    ...
    local bd = math.max(1, get_hero_attr('鐢熷懡缁撶畻锟?, '鐢熷懡'))
    return bc, bd
  end;
  ...
  local function get_hero_item_by_slot(bm)
    ...
    local bn,
    bo = pcall(STATE.hero.get_item_by_slot, STATE.hero, '鐗╁搧锟?, bm)
  ...
  local function format_gear_upgrade_tip(bs)
    if not bs then return '褰撳墠娌℃湁鎴愰暱姝﹀櫒鏁版嵁锟? end;
    ...
    if #bt == 0 then return '褰撳墠娌℃湁鎴愰暱姝﹀櫒鏁版嵁锟? end;
  ...
  local function get_item_display_info(bo)
    ...
    local bz = bo.get_name and bo:get_name() or '鐗╁搧'
    local bt = {}
    ...
    if bB and bB > 1 then bt[#bt + 1] = string.format('灞傛暟锟?d', bB) end;

    local bC = bo.get_charge and tonumber(bo:get_charge()) or 0;
    if bC and bC > 0 then bt[#bt + 1] = string.format('鍏呰兘锟?d', bC) end;
    if #bt == 0 then bt[#bt + 1] = '褰撳墠娌℃湁棰濆璇存槑锟? end;
  ...
  local function build_weapon_tooltip()
    ...
    return {
      title = tostring(bs.title_text or '鎴愰暱姝﹀櫒'),
      subtitle = tostring(bs.subtitle_text or ''),
      body = tostring(bs.cost_text or ''),
      icon = bs.icon_res,
      tip_model = {
        item_name_text = tostring(bs.title_text or '鎴愰暱姝﹀櫒'),
        quality_text = tostring(bs.subtitle_text or ''),
        effect_body_text = tostring(bs.cost_text or ''),
        set_title_text = '褰撳墠灞炴€у锟?,
        set_body_lines = #attr_lines > 0 and attr_lines or { '褰撳墠鏃犵洿鎺ュ睘鎬у锟? },
        bonus_lines = #affix_lines > 0 and affix_lines or { '鏆傛棤璇嶇紑' },
  ...
  local function build_bond_slot_tooltip(bL)
    ...
    local bz = tostring(bM.item_name_text or bs.title_text or '娴佹淳')
    local bN = {}
    ...
    if bO ~= '' or bP ~= '' then bN[#bN + 1] = '娴佹淳锟? .. bO .. bP end;
    ...
    if bM.set_body_lines and #bM.set_body_lines > 0 then
      ...
      local bQ = tostring(bM.set_title_text or ''):gsub('锟?$', '')
      bt[#bt + 1] = '[' .. (bQ ~= '' and bQ or bond_skill_label) .. ']'
      append_lines(bt, bM.set_body_lines)
    end;
    if #bt == 0 then bt[#bt + 1] = '褰撳墠娌℃湁娴佹淳璇存槑锟? end;
  ...
  local function build_draw_tooltip()
    local bt = { '[宸﹂敭鐐瑰嚮]', string.format('鏈娑堬拷?%d 涓湪锟?, P), string.format('褰撳墠鎷ユ湁 %s 鏈ㄦ潗',
      format_short_number(STATE.resources and STATE.resources.wood or 0)), '', '鎶藉彇娴佹淳鍗＄墝锛岀浉鍚屾祦娲句細鑷姩鏀跺綍杩涘崱鍐岋拷? }
    return {
      title = '鎶藉崱 - [蹇嵎閿細F]',
      subtitle = '',
      body = table.concat(bt, '\n'),
  ...
  local function build_hero_catalog_tooltip()
    local bt = { '[宸﹂敭鐐瑰嚮]', '鎵撳紑鑻遍泟鍥鹃壌闈㈡澘锟?, '鍙煡鐪嬭嫳闆勫畾浣嶃€佹牳蹇冩妧鑳戒笌鏄熺骇锟? }
    return {
      title = '濡備綍鍙樺己',
      subtitle = '',
      body = table.concat(bt, '\n'),
  ...
  local function build_evolution_entry_tooltip()
    local bt = { '[宸﹂敭鐐瑰嚮]', '宸叉敼涓烘柊鑻遍泟鍔熻兘鍏ュ彛锟?, '璇风偣鍑烩€滃浣曞彉寮衡€濊繘鍏ヨ嫳闆勫浘閴达拷? }
    return {
      title = '鑻遍泟杩涢樁鍏ュ彛 - [蹇嵎閿細H]',
      subtitle = '',
      body = table.concat(bt, '\n'),
      icon = nil
    }
  ...
  local function build_save_panel_tooltip()
    local bt = { '[宸﹂敭鐐瑰嚮]', '鎵撳紑瀛樻。闈㈡澘锛涘鏋滃綋鍓嶆病鏈夊彲鎵撳紑鐨勫瓨妗ｇ晫闈紝鍒欐樉绀鸿繍琛屾椂鐘舵€侊拷? }
    return {
      title = '瀛樻。 - [蹇嵎閿細P]',
      subtitle = '',
      body = table.concat(bt, '\n'),
  ...
  local function build_consumable_tooltip(bm)
    if bm == 1 then
      return {
        title = '灞炴€у疂锟?,
        subtitle = '绫诲瀷锛氭秷鑰楀搧',
        body = table.concat({ '[鐐瑰嚮浣跨敤]', '鍙€夋嫨涓€鏉￠殢鏈哄睘鎬у己鍖栵拷?, '鑻遍泟姣忓崌 5 绾э紝鎴栧畬鎴愬疂鐭虫寫鎴樻椂锛屽彲鑾峰緱 1 棰楋拷? }, '\n'),
        icon = 300540000
      }
    end;
  ...
  local function build_attack_skill_entry(slot_data, slot_index)
    ...
    if damage_ratio > 0 then tip_lines[#tip_lines + 1] = string.format('鍊嶇巼锟?.0f%%', damage_ratio * 100) end;

    local cast_range = math.max(0, tonumber(slot_data.cast_range or 0) + tonumber(slot_data.range_bonus or 0))
    if cast_range > 0 then tip_lines[#tip_lines + 1] = string.format('灏勭▼锟?d', math.floor(cast_range + 0.5)) end;

    local base_cooldown = tonumber(slot_data.base_cooldown) or 0;
    if base_cooldown > 0 then tip_lines[#tip_lines + 1] = string.format('鍩虹鍐峰嵈锟?.1fs', base_cooldown) end;
    ...
    return {
      ...
      name = tostring(slot_data.name or slot_data.id or '鎶€锟? .. tostring(slot_index)),
      icon = resolve_display_icon(slot_data.icon_res, slot_data.ui_icon, slot_data.icon, slot_data.bg),
      key = tostring(slot_index),
      cooldown_text = cooldown_remaining > 0 and string.format('%.1fs', cooldown_remaining) or '灏辩华',
      ...
      stack_text = '',
      tip_title = tostring(slot_data.name or slot_data.id or '鎶€锟?),
      tip_text = #tip_lines > 0 and table.concat(tip_lines, '\n') or '褰撳墠娌℃湁鎶€鑳借鏄庯拷?
    }
  end;

  local function normalize_rarity_display(rarity_key) return RARITY_NAME_MAP[rarity_key] or '鏅拷? end;
  ...
  local function build_evolution_skill_entry(evo_def, slot_index)
    ...
    local display_name = roster_entry and roster_entry.name or evo_def.name or '涓撶簿' .. tostring(slot_index)
    local display_title = roster_entry and roster_entry.title or '鑻遍泟鐪熻韩'
    ...
    return {
      ...
      stack_text = '',
      tip_title = string.format('%s路%s', tostring(display_name), tostring(display_title)),
  ...
  local function get_pending_choice_status()
    if STATE.gear_state and STATE.gear_state.awaiting_choice and STATE.gear_state.current_choices then return '姝﹀櫒寰咃拷?,
          '鎴愰暱姝﹀櫒璇嶇紑鍊欓€夊凡鍑虹幇锛岃鐐瑰嚮闈㈡澘瀹屾垚閫夋嫨锟? end;

    if STATE.bond_runtime and STATE.bond_runtime.awaiting_choice and STATE.bond_runtime.current_choices then return '娴佹淳寰咃拷?,
          '娴佹淳鍊欓€夊凡鐢熸垚锛岃鐐瑰嚮闈㈡澘瀹屾垚閫夋嫨锟? end;

    local ct = STATE.evolution_runtime;
    if ct and ct.awaiting_choice and ct.current_choices then return '鑻遍泟鍔熻兘鎻愮ず', '杩涢樁宸茶縼绉诲埌鏂拌嫳闆勫姛鑳斤紝璇锋墦寮€鑻遍泟鍥鹃壌鏌ョ湅锟? end;
  ...
  local function get_current_tip_text()
    ...
    if hud_state.tip_panel and hud_state.tip_expires_at and hud_state.tip_expires_at > (STATE.runtime_elapsed or 0) then return
      hud_state.tip_title_text ~= '' and hud_state.tip_title_text or '绯荤粺鎻愮ず', hud_state.tip_body_text or '' end;
    ...
    if ck and #ck > 0 then
      ...
      if c4 and c4.text and c4.text ~= '' then
        if c4.style == 'reward' then return '濂栧姳鎻愮ず', c4.text end;

        if c4.style == 'warning' then return '鎴樻枟璀︽姤', c4.text end;

        if c4.style == 'rare' then return '绋€鏈変簨锟?, c4.text end;

        if c4.style == 'positive' then return '杩涘害鏇存柊', c4.text end;

        return '绯荤粺娑堟伅', c4.text
      end
    end;

    return '鎿嶄綔鎻愮ず', 'F 鎶藉崱锛屽浣曞彉寮烘煡鐪嬭嫳闆勫浘閴达紝H 鏌ョ湅鑻遍泟鍔熻兘锛孭 鎵撳紑瀛樻。锟?
  end;
  ...
  local function get_status_bar_text()
    ...
    if choice_label then return '鐘舵€侊細' .. cv end;

    local damage_text_status = X.hide_damage_text and '璺冲瓧锟? or '璺冲瓧寮€'
    local hit_effects_status = X.hide_hit_effects and '鐗规晥锟? or '鐗规晥寮€'
    local pause_status = X.soft_paused and '宸叉殏锟? or '杩涜锟?
    return string.format('鐘舵€侊細%s | %s | %s', cA, cy, cz)
  end;

  local function get_station_hint_text() return '鎸塅鎶藉崱锛涚偣鍑诲浣曞彉寮烘煡鐪嬭嫳闆勫浘锟? end;

  local function get_hotkey_help_text() return table.concat(
    { 'F / 鎶藉崱锛氭祦娲句笁閫変竴', '濡備綍鍙樺己锛氭煡鐪嬭嫳闆勫浘锟?, 'H / 鑻遍泟鍔熻兘锛氭煡鐪嬪浘閴翠笌鎴愰暱', 'Q / W / E锛氳瘯鐐煎叆锟?, 'TAB / T锛氬睘鎬ч潰锟?, 'SPACE锛氭墦鍗扮姸鎬佹锟?, 'P锛氭墦寮€瀛樻。' },
      '\n') end;
  ...
  local function resolve_static_ui_panels()
    ...
    set_ui_text_alignment(hud_state.attr_panel_title, '锟?, '锟?)
    set_ui_text_alignment(hud_state.attr_panel_body, '锟?, '锟?)
    set_ui_text_alignment(hud_state.attr_panel_hint, '锟?, '锟?)
    if hud_state.bound_events.static_attr_panel_close ~= hud_state.attr_panel and is_ui_alive(hud_state.attr_panel) then
      hud_state.bound_events.static_attr_panel_close = hud_state.attr_panel; hud_state.attr_panel:add_fast_event('宸﹂敭-鐐瑰嚮', function()
    ...
    set_ui_text_alignment(hud_state.tip_panel_title, '锟?, '锟?)
    set_ui_text_alignment(hud_state.tip_panel_body, '锟?, '锟?)
    set_ui_text_alignment(hud_state.tip_panel_hint, '锟?, '锟?)
    if hud_state.bound_events.static_tip_panel_close ~= hud_state.tip_panel and is_ui_alive(hud_state.tip_panel) then
      hud_state.bound_events.static_tip_panel_close = hud_state.tip_panel; hud_state.tip_panel:add_fast_event('宸﹂敭-鐐瑰嚮', function()
        local hud_state_inner = get_hud_state()
    ...
    set_ui_text_alignment(hud_state.hover_tip_panel_title, '锟?, '锟?)
    set_ui_text_alignment(hud_state.hover_tip_panel_subtitle, '锟?, '锟?)
    set_ui_text_alignment(hud_state.hover_tip_panel_body, '锟?, '锟?)
    ...
    hud_state.bond_tip_panel = resolve_first_ui_node({ 'BattleDetailTipsPanel.detail_panel', 'TipsPanel.璇︽儏闈㈡澘', '璇︽儏闈㈡澘' })
    ...
    hud_state.bond_tip_title = resolve_first_ui_node({ 'BattleDetailTipsPanel.detail_panel.title', 'TipsPanel.璇︽儏闈㈡澘.鍒楄〃.鏍囬', '璇︽儏闈㈡澘.鍒楄〃.鏍囬' })
    hud_state.bond_tip_subtitle = resolve_first_ui_node({ 'BattleDetailTipsPanel.detail_panel.subtitle', 'TipsPanel.璇︽儏闈㈡澘.鍒楄〃.鍓爣锟?, '璇︽儏闈㈡澘.鍒楄〃.鍓爣锟? })
    ...
    for i = 1, 5 do
      hud_state.bond_tip_contents[i] = resolve_first_ui_node({
        ...
        'TipsPanel.璇︽儏闈㈡澘.鍒楄〃.鍐呭' .. tostring(i),
        '璇︽儏闈㈡澘.鍒楄〃.鍐呭' .. tostring(i)
      })
    end
    hud_state.bond_tip_bottom = resolve_first_ui_node({ 'BattleDetailTipsPanel.detail_panel.bottom', 'TipsPanel.璇︽儏闈㈡澘.鍒楄〃.搴曢儴鍐呭', '璇︽儏闈㈡澘.鍒楄〃.搴曢儴鍐呭' })
    hud_state.bond_tip_icon = resolve_first_ui_node({ 'BattleDetailTipsPanel.detail_panel.icon_box.icon', 'TipsPanel.璇︽儏闈㈡澘.鍥炬爣.鍥炬爣', '璇︽儏闈㈡澘.鍥炬爣.鍥炬爣' })
    hud_state.bond_tip_icon_name = resolve_first_ui_node({ 'BattleDetailTipsPanel.detail_panel.icon_name', 'TipsPanel.璇︽儏闈㈡澘.鍥炬爣.鍚嶇О', '璇︽儏闈㈡澘.鍥炬爣.鍚嶇О' })
    if not is_ui_alive(hud_state.bond_tip_panel) then
      ...
      if is_ui_alive(cE) then hud_state.bond_tip_panel = ui_root.resolve_child(cE, '璇︽儏闈㈡澘') end
    end;
    if is_ui_alive(hud_state.bond_tip_panel) then
      ...
      if not is_ui_alive(hud_state.bond_tip_title) then hud_state.bond_tip_title = ui_root.resolve_child(hud_state.bond_tip_panel, 'title') or ui_root.resolve_child(hud_state.bond_tip_panel, '鍒楄〃.鏍囬') end;
      if not is_ui_alive(hud_state.bond_tip_subtitle) then hud_state.bond_tip_subtitle = ui_root.resolve_child(hud_state.bond_tip_panel, 'subtitle') or ui_root.resolve_child(hud_state.bond_tip_panel, '鍒楄〃.鍓爣锟?) end;
      if not is_ui_alive(hud_state.bond_tip_bottom) then hud_state.bond_tip_bottom = ui_root.resolve_child(hud_state.bond_tip_panel, 'bottom') or ui_root.resolve_child(hud_state.bond_tip_panel, '鍒楄〃.搴曢儴鍐呭') end;
      ...
      for i = 1, 5 do
        if not is_ui_alive(hud_state.bond_tip_contents[i]) then hud_state.bond_tip_contents[i] = ui_root.resolve_child(hud_state.bond_tip_panel, 'content_' .. tostring(i)) or ui_root.resolve_child(hud_state.bond_tip_panel, '鍒楄〃.鍐呭' .. tostring(i)) end;
      end
      if not is_ui_alive(hud_state.bond_tip_icon) then hud_state.bond_tip_icon = ui_root.resolve_child(hud_state.bond_tip_panel, 'icon_box.icon') or ui_root.resolve_child(hud_state.bond_tip_panel, '鍥炬爣.鍥炬爣') end;
      if not is_ui_alive(hud_state.bond_tip_icon_name) then hud_state.bond_tip_icon_name = ui_root.resolve_child(hud_state.bond_tip_panel, 'icon_name') or ui_root.resolve_child(hud_state.bond_tip_panel, '鍥炬爣.鍚嶇О') end;
    end;
    ...
    if not is_ui_alive(hud_state.big_cursor) then
      ...
      local ac = cF:create_child('鏂囨湰')
      ac:set_ui_size(60, 60)
      ac:set_text('锟?)
      ac:set_font_size(28)
      ac:set_text_color(255, 233, 158, 235)
      ac:set_text_alignment('锟?, '锟?)
      ac:set_z_order(9380)
  ...
  local function show_hover_tip_payload(bs)
    ...
    if not is_ui_alive(hud_state.hover_tip_panel) then
      local dA3 = tostring(bs.title or '璇存槑')
    ...
    hud_state.hover_tip_visible = true; set_ui_text(hud_state.hover_tip_panel_title, bs.title or '璇存槑')
  ...
  -- 閫氱敤璇︽儏闈㈡澘娓叉煋锛歿 title, subtitle, icon, icon_name, contents={...}, bottom }
  ...
  local function show_bond_tip_payload(bs)
    ...
    show_detail_payload({
      title = tostring(bM.item_name_text or bs.title or '缇佺粖锟?),
  ...
  local function get_or_create_hero_model_ui()
    ...
    local bn,
    cR = pcall(cQ.create_child, cQ, '妯″瀷')
  ...
  local function bind_click_handler(cT, u, cU)
    ...
    u:add_fast_event('宸﹂敭-鐐瑰嚮', function()
      if S then S() end;
      cU()
    end)
  end;
  ...
  local function bind_hover_handlers(cT, u, cW, cX)
    ...
    if not is_ui_alive(u) or not u.add_fast_event then return end;
    hud_state.bound_events[cT] = u; safe_ui_call(u, 'set_intercepts_operations', true)
    u:add_fast_event('榧犳爣-绉诲叆', function()
      if cW then cW(u) end
    end)
    u:add_fast_event('榧犳爣-绉诲嚭', function()
      if cX then cX(u) end
    end)
  end;
  ...
  local function show_tip_panel(ac, c_, bz)
    ...
    hud_state.tip_title_text = bz or '绯荤粺鎻愮ず'
    hud_state.tip_body_text = tostring(ac or '')
    set_ui_text(hud_state.tip_panel_title, bz or '绯荤粺鎻愮ず')
  ...
  local function toggle_big_cursor()
    ...
    show_tip_panel(X.big_cursor and '澶ч紶鏍囧凡寮€鍚紝榧犳爣浣嶇疆浼氭樉绀鸿緟鍔╁湀锟? or '澶ч紶鏍囧凡鍏抽棴锟?, 4, '榧犳爣杈呭姪')
  end;

  local function toggle_damage_text_visible()
    local X = ensure_ui_preferences()
    X.hide_damage_text = not X.hide_damage_text; show_tip_panel(X.hide_damage_text and '宸插睆钄借烦瀛楋拷? or '宸叉仮澶嶈烦瀛楁樉绀猴拷?, 4, '鏈湴鏄剧ず')
  end;

  local function toggle_hit_effects_visible()
    local X = ensure_ui_preferences()
    X.hide_hit_effects = not X.hide_hit_effects; show_tip_panel(X.hide_hit_effects and '宸插睆钄藉眬鍐呮妧鑳界壒鏁堬拷? or '宸叉仮澶嶅眬鍐呮妧鑳界壒鏁堬拷?, 4,
      '鏈湴鏄剧ず')
  end;
  ...
  local function toggle_soft_pause()
    ...
    if X.soft_paused then
      ...
      show_tip_panel('瀵瑰眬宸叉殏鍋滐紝鍐嶇偣涓€娆＄户缁拷?, 4, '鎴樻枟鎺у埗')
    else
      y3.game.resume_soft_pause()
      show_tip_panel('瀵瑰眬宸茬户缁拷?, 4, '鎴樻枟鎺у埗')
    end
  end;
  ...
  local function toggle_runtime_attr_panel()
    ...
    if hud_state.attr_panel_visible then
      local d9 = M and M() or {
        string.format('绛夌骇锟?d', get_hero_level()),
        string.format('鏀诲嚮锟?s', format_short_number(get_hero_attr('鏀诲嚮缁撶畻锟?, '鏀诲嚮'))),
        string.format('鎶ょ敳锟?s', format_short_number(get_hero_attr('鎶ょ敳缁撶畻锟?, '鎶ょ敳'))),
        string.format('鍔涢噺锟?s', format_short_number(get_hero_attr('鏈€缁堝姏锟?, '鍔涢噺'))),
        string.format('鏅哄姏锟?s', format_short_number(get_hero_attr('鏈€缁堟櫤锟?, '鏅哄姏'))),
        string.format('鏁忔嵎锟?s', format_short_number(get_hero_attr('鏈€缁堟晱锟?, '鏁忔嵎'))),
      }
      set_ui_text(hud_state.attr_panel_title, '灞炴€ф€昏')
  ...
  local function set_static_labels()
    local X = ensure_ui_preferences()
    set_ui_text(resolve_ui_node('top.top.left_buttons.btn_exit'), '閫€锟?)
    set_ui_text(resolve_ui_node('top.top.left_buttons.btn_setting'), '璁剧疆')
    set_ui_text(resolve_ui_node('top.top.left_buttons.btn_save'), '瀛樻。')
    set_ui_text(resolve_ui_node('top.top.left_buttons.btn_pause'), X.soft_paused and '缁х画' or '鏆傚仠')
    ...
    set_ui_text(resolve_ui_node('top.top.left_buttons.btn_hotkey'), '閿綅')
    set_ui_visible(resolve_ui_node('BattleBottomHUD.layout.left_station.toggle_frame'), false)
    set_ui_text(resolve_ui_node('BattleBottomHUD.layout.right_station.card_panel.draw_button.button'), '鎶藉崱')
    set_ui_text(resolve_ui_node('BattleBottomHUD.layout.right_station.card_panel.reward_button.button'), '濡備綍鍙樺己')
    set_ui_text(resolve_ui_node('BattleBottomHUD.layout.right_station.card_panel.kill_reward_button.button'), '鏉€锟?)
    set_ui_text(resolve_ui_node('BattleBottomHUD.layout.right_station.card_panel.fish_button.button'), '閽撻奔')
  ...
  local function show_weapon_tip()
    ...
    show_detail_payload({
      ...
      contents = {
        ...
        '[褰撳墠灞炴€у骞匽\n' .. join_or_default(attr_lines, '褰撳墠鏃犵洿鎺ュ睘鎬у锟?),
        join_or_default(affix_lines, '鏆傛棤璇嶇紑'),
        '锟?10 绾т細杩涘叆涓€娆¤瘝缂€閫夋嫨锛岄€夋嫨瀹屾垚鍚庡彲缁х画鍗囩骇锟?,
      },
      bottom = '鎮仠瑁呭鏍忔垚闀挎鍣ㄦ椂瀹炴椂璇诲彇褰撳墠绛夌骇銆佽垂鐢ㄣ€佸睘鎬у拰璇嶇紑锟?
    })
    local hud_state = get_hud_state()
  ...
  local function show_hero_skill_tip(bm, cj)
    ...
    show_detail_payload({
      title = tostring(c4.tip_title or c4.name or '鑻遍泟鎶€锟?),
      subtitle = tostring(c4.badge_text or c4.quality or c4.cooldown_text or ''),
      icon = c4.icon,
      icon_name = tostring(c4.name or ''),
      contents = {
        join_or_default({ tip_lines[1], tip_lines[2] }, '褰撳墠娌℃湁鎶€鑳借鏄庯拷?),
        join_or_default({ tip_lines[3], tip_lines[4] }, ''),
        c4.cooldown_text and c4.cooldown_text ~= '' and ('鐘舵€侊細' .. tostring(c4.cooldown_text)) or '',
        c4.stack_text and c4.stack_text ~= '' and ('璁℃暟锟? .. tostring(c4.stack_text)) or '',
      },
      bottom = c4.key and ('鎶€鑳戒綅锟? .. tostring(c4.key)) or ''
    })
  end;
  ...
  local function show_skill_tip(bm, cj)
    ...
    show_detail_payload({
      title = tostring(c4.tip_title or c4.name or '鎶€锟?),
  ...
  local function show_evolution_tip(bm)
    ...
    show_detail_payload({
      title = tostring(c4.tip_title or c4.name or '杩涘寲'),
  ...
  local function show_buff_tip(bm)
    ...
    show_detail_payload({
      title = tostring(c4.tip_title or c4.name or '榄旀硶鏁堟灉'),
      subtitle = tostring(c4.badge_text or ''),
      icon = c4.icon,
      icon_name = tostring(c4.name or ''),
      contents = { tostring(c4.tip_text or '') },
  ...
  local function show_bond_slot_tip(bm)
    ...
    show_hover_tip_payload({ title = '缇佺粖锟?' .. tostring(bm), subtitle = '', body = '褰撳墠妲戒綅鏆傛棤缇佺粖璇存槑锟?, icon = nil })
  end;
  ...
  local function refresh_loadout_row()
    ...
    resolve_ui_node('BattleBottomHUD.layout.right_station.loadout_row.loadout_title'), '鐗╁搧锟?)
    set_ui_size(resolve_ui_node('BattleBottomHUD.layout.right_station.loadout_row.loadout_title'), 92, 17)
    set_ui_text_alignment(resolve_ui_node('BattleBottomHUD.layout.right_station.loadout_row.loadout_title'), '锟?, '锟?)
    for bL = 1, 6 do
  ...
  ensure_hud = function()
    ...
    bind_click_handler('top_hotkey', resolve_ui_node('top.top.left_buttons.btn_hotkey'), function()
      show_tip_panel(get_hotkey_help_text(), 10, '蹇嵎锟?)
    end)
    ...
    for bL = 1, BOND_CARD_SLOT_COUNT do
      ...
      bind_click_handler('battle_bond_slot_' .. tostring(bL), dA4, function()
        show_tip_panel('bond_progress 鍔熻兘宸茬Щ闄わ拷?, 4, '鎻愮ず')
      end)
    end;
  ...
  local function refresh_top_bar()
    set_ui_text(resolve_ui_node('top.top.閲戝竵.image_3.label_2'), format_short_number(STATE.resources and STATE.resources.gold or 0))
    set_ui_text(resolve_ui_node('top.top.鏈ㄦ潗.image_3.label_2'), format_short_number(STATE.resources and STATE.resources.wood or 0))
    set_ui_text(resolve_ui_node('top.top.浜哄彛.image_3.label_2'), format_short_number(STATE.total_kills or 0))
    set_ui_text(resolve_ui_node('top.top.閲戝竵.delta'), string.format('+%s/s', format_short_number(get_hero_attr('姣忕閲戝竵'))))
    set_ui_text(resolve_ui_node('top.top.鏈ㄦ潗.delta'), string.format('+%s/s', format_short_number(get_hero_attr('姣忕鏈ㄦ潗'))))
    set_ui_text(resolve_ui_node('top.top.浜哄彛.delta'), string.format('锟?%d', math.max(0, tonumber(STATE.total_enemy_alive) or 0)))
    local dt,
    ...
    local dv = STATE.current_stage_def and (STATE.current_stage_def.display_label or STATE.current_stage_def.display_name) or '褰撳墠绔犺妭'
    local dw = STATE.current_mode_def and STATE.current_mode_def.display_name or '鎴樻枟妯″紡'
    local dx = STATE.active_wave and STATE.active_wave.wave and STATE.active_wave.wave.name or
    (STATE.current_wave_index and STATE.current_wave_index > 0 and string.format('锟?d锟?, STATE.current_wave_index) or '鏈紑锟?)
    local dy = ({ get_pending_choice_status() })[1] or (STATE.session_phase == 'battle' and '鎴樻枟锟? or '鍑嗗锟?)
    local dz;
    if STATE.active_wave and STATE.active_wave.wave and STATE.active_wave.wave.boss_spawn_sec and STATE.active_wave.boss_spawned ~= true then
      ...
      dz = string.format('鏁屼汉 %d', math.max(0, tonumber(STATE.total_enemy_alive) or 0))
    end;
    ...
    set_ui_text(resolve_ui_node('top.top.scoreboard.title'), '鐜╁鐘讹拷?)
    set_ui_text(resolve_ui_node('top.top.scoreboard.player_name'), get_player_name())
    set_ui_text(resolve_ui_node('top.top.scoreboard.player_power'), format_short_number(get_hero_attr('鏀诲嚮缁撶畻锟?, '鏀诲嚮')))
    set_ui_text(resolve_ui_node('top.top.scoreboard.player_state'), STATE.session_phase == 'battle' and '鎴樻枟锟? or '灞€锟?)
  ...
  local function refresh_player_attr_list()
    local dB = format_short_number(get_hero_attr('鏀诲嚮缁撶畻锟?, '鏀诲嚮'))
    local dC = format_short_number(get_hero_attr('鎶ょ敳缁撶畻锟?, '鎶ょ敳'))
    local dD = { {
      label = '鎴樺姏',
      value = dB,
      delta = ''
    },
      {
        label = '鏀诲嚮',
        value = dB,
        delta = format_percent_delta(get_hero_attr('鏀诲嚮澧炲箙'), get_hero_attr('鏈€缁堟敾锟?))
      },
      {
        label = '鎶ょ敳',
        value = dC,
        delta = format_percent_delta(get_hero_attr('鎶ょ敳澧炲箙'), get_hero_attr('鏈€缁堟姢锟?))
      },
      {
        label = '鍔涢噺',
        value = format_short_number(get_hero_attr('鏈€缁堝姏锟?, '鍔涢噺')),
        delta = format_percent_delta(get_hero_attr('鍔涢噺澧炲箙'), get_hero_attr('鏈€缁堝姏閲忓锟?))
      },
      {
        label = '鏅哄姏',
        value = format_short_number(get_hero_attr('鏈€缁堟櫤锟?, '鏅哄姏')),
        delta = format_percent_delta(get_hero_attr('鏅哄姏澧炲箙'), get_hero_attr('鏈€缁堟櫤鍔涘锟?))
      },
      {
        label = '鏁忔嵎',
        value = format_short_number(get_hero_attr('鏈€缁堟晱锟?, '鏁忔嵎')),
        delta = format_percent_delta(get_hero_attr('鏁忔嵎澧炲箙'), get_hero_attr('鏈€缁堟晱鎹峰锟?))
      } }
  ...
  local function refresh_hero_panel()
    ...
    set_ui_text_alignment(resolve_ui_node('BattleBottomHUD.layout.center_hub.hero_panel.hero_name'), '锟?, '锟?)
    ...
    set_ui_text_alignment(resolve_ui_node('BattleBottomHUD.layout.center_hub.hero_panel.hero_hp_text'), '锟?, '锟?)
    ...
    set_ui_text(resolve_combat_module_ui('exp_bar.level_label'), string.format('绛夌骇锟?d', get_hero_level()))
    ...
    set_ui_text_alignment(resolve_combat_module_ui('exp_bar.exp_text'), '锟?, '锟?)
  end;
  ...
  local function refresh_challenge_row()
    ...
    resolve_combat_module_ui('challenge_row.gold_trial.title'), '閲戝竵鎸戞垬')
    set_ui_text(resolve_combat_module_ui('challenge_row.gold_trial.count'), tostring(math.max(0, tonumber(dP) or 0)))
    set_ui_text(resolve_combat_module_ui('challenge_row.treasure_trial.title'), '鏈ㄦ潗鎸戞垬')
    set_ui_text(resolve_combat_module_ui('challenge_row.treasure_trial.count'), tostring(math.max(0, tonumber(dQ) or 0)))
    set_ui_text(resolve_combat_module_ui('challenge_row.climb_layer.title'), '褰撳墠娉㈡')
    ...
    set_ui_text(resolve_combat_module_ui('challenge_row.realm_progress.title'), '瀛樻椿鏁屼汉')
  ...
  local function refresh_status_text()
    ...
    set_ui_text(resolve_combat_module_ui('status_text'), '鐘舵€侊細')
    set_ui_text_alignment(resolve_combat_module_ui('status_text'), '锟?, '锟?)
    set_ui_text(resolve_ui_node('BattleBottomHUD.layout.right_station.card_panel.station_hint'), get_station_hint_text())
    set_ui_text_alignment(resolve_ui_node('BattleBottomHUD.layout.right_station.card_panel.station_hint'), '锟?, '锟?)
  end;
  ...
  local function show_bond_replacement_panel()
    ...
    if not ok_panel or not panel then
      message_fn('鏈壘鍒扮緛缁婃浛鎹㈤潰鏉匡細' .. BOND_REPLACEMENT_PANEL)
      return
    end
    set_ui_visible(panel, true)
    local new_card = info.new_card
    local title_text = string.format('閫夋嫨瑕佹浛鎹㈢殑缇佺粖锛堣幏寰楋細%s锟?, new_card.name or '鏈煡鍗＄墝')
    local desc_text = string.format('缇佺粖宸叉弧锛岃閫夋嫨瑕佹浛鎹㈢殑缇佺粖锛歕n鏂扮緛缁婏細%s锟?s锟?, new_card.name or '', new_card.bond_name or '')
    local title_ui = nil
    local desc_ui = nil
    ...
    for slot_index = 1, 8 do
      ...
      if ok_slot and slot_ui then
        if option then
          ...
          local icon_path = slot_path .. '.icon'
          local name_path = slot_path .. '.name'
          ...
          click_handler = function()
            if apply_bond_replacement_fn then
              ...
              if ok then
                ...
                message_fn(string.format('宸叉浛鎹㈢緛缁婏細%s 锟?%s', option.display_name, new_card.name))
                if refresh_hud then refresh_hud() end
              end
            end
          end
          local ok_click, existing = pcall(slot_ui.add_fast_event, slot_ui, '宸﹂敭-鐐瑰嚮')
          if ok_click then
          end
          local ok_bind, _ = pcall(function()
            slot_ui:set_intercepts_operations(true)
            slot_ui:add_fast_event('宸﹂敭-鐐瑰嚮', click_handler)
          end)
        else
    ...
    if ok_cancel and cancel_ui then
      local cancel_handler = function()
        ...
        bond_replacement_visible = false
        set_ui_visible(panel, false)
        message_fn('宸插彇娑堢緛缁婃浛锟?)
      end
      local ok_bind, _ = pcall(function()
        cancel_ui:set_intercepts_operations(true)
        cancel_ui:add_fast_event('宸﹂敭-鐐瑰嚮', cancel_handler)
      end)
    end
  end
